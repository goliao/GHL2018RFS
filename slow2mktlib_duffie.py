...
def funZeroDuffie(v0,k,tau,sigmasA,sigmasB,q,smA,smB,rhosA,rhosB,rhor,rhoz,sigmar,sigmaz,rm,zm,DA,DB):
    ...
    V1=(thetaA/(1-thetaA))**2*alphaA1.T.dot(Sigma).dot(alphaA1) 
    V2=(thetaA/(1-thetaA))*alphaA1.T.dot(Sigma).dot(thetaB/(1-thetaB)*alphaB1+ez) 
    V4=(thetaB/(1-thetaB)*alphaB1+ez).T.dot(Sigma).dot(thetaB/(1-thetaB)*alphaB1+ez)
    V3=(thetaA/(1-thetaA))*alphaA1.T.dot(Sigma).dot(thetaB/(1-thetaB)*alphaB1+ez) 
    Vmat=np.array([[V1,V2],[V3,V4]])
    ErxkACond=(alphaA1-er).T.dot(inv(I-Gamma))+thetaA/(1-thetaA)*alphaA1.T 
    ErxkBCond=(alphaB1-er).T.dot(inv(I-Gamma))+thetaB/(1-thetaB)*alphaB1.T-ez.T.dot(inv(I-Gamma)).dot(Gamma) 
    # newalphaA1=(1-thetaA)/(1-thetaA*rhor)*er+VA1/(tau*q)*((1-thetaA)/(1-thetaA*rhosA)*esA-(1-thetaA)*(1-q)/k*inv(I-thetaA*Gamma.T).dot(onesA+deltaA1)) 
    # newalphaB1=(1-thetaB)/(1-rhor*thetaB)*er+(1-thetaB)/(1-rhoz*thetaB)*rhoz*ez+VB1/(tau*q)*((1-thetaB)/(1-thetaB*rhosB)*esB-(1-thetaB)*(1-q)/k*inv(I-thetaB*Gamma.T).dot(onesB+deltaB1)) 
    Er=np.array([er,er])
    Ez=np.array([ez*0,ez])
    Ss=np.array([esA,esB])
    Sd=np.array([onesA,onesB])
    D=np.array([deltaA1,deltaB1])
    A=(1-thetaA)*Er.dot(inv(I-thetaA*Gamma))+(1-thetaA)*Ez.dot(Gamma).dot(inv(I-thetaA*Gamma))+\
      (1-thetaA)/tau/q*Vmat.dot(Ss-(1-q)/k*(Sd+D)).dot(inv(I-thetaA*Gamma))

    ## our version that mimicks duffie
    if (1==0):
      qA=q*0.5
      qB=q*0.5
      qmat=diag([qA,qB])
      Vmat=diag(diag(Vmat))
      A=(1-thetaA)*Er.dot(inv(I-thetaA*Gamma))+\
          (1-thetaA)*Ez.dot(Gamma).dot(inv(I-thetaA*Gamma))+\
          (1-thetaA)/tau*(inv(qmat)*Vmat).dot(Ss-(1-q)/k*(Sd+D)).dot(inv(I-thetaA*Gamma))
          
    newalphaA1,newalphaB1=vsplit(A,2)
    newalphaA1=newalphaA1[0]
    newalphaB1=newalphaB1[0]

    newdeltaA1=tau/(VAk*VBk-CABk**2)*(VBk*ErxkACond-CABk*ErxkBCond).dot(I-matrix_power(Gamma,int(k))).T 
    newdeltaB1=tau/(VAk*VBk-CABk**2)*(VAk*ErxkBCond-CABk*ErxkACond).dot(I-matrix_power(Gamma,int(k))).T 
...
def funZeroConstDuffie(w0,v,k,tau,sigmasA,sigmasB,q,smA,smB,rhosA,rhosB,rhor,rhoz,sigmar,sigmaz,rm,zm,DA,DB,cash_cov=0):
    ...
    #VA1=(thetaA/(1-thetaA))**2*alphaA1.T.dot(Sigma).dot(alphaA1) 
    #VB1=(thetaB/(1-thetaB)*alphaB1+ez).T.dot(Sigma).dot(thetaB/(1-thetaB)*alphaB1+ez) 
    V1=(thetaA/(1-thetaA))**2*alphaA1.T.dot(Sigma).dot(alphaA1) 
    V2=(thetaA/(1-thetaA))*alphaA1.T.dot(Sigma).dot(thetaB/(1-thetaB)*alphaB1+ez) 
    V4=(thetaB/(1-thetaB)*alphaB1+ez).T.dot(Sigma).dot(thetaB/(1-thetaB)*alphaB1+ez)
    V3=(thetaA/(1-thetaA))*alphaA1.T.dot(Sigma).dot(thetaB/(1-thetaB)*alphaB1+ez) 
    ...
    Ss=np.array([esA,esB])
    Sd=np.array([onesA,onesB])
    d0=np.array([deltaA0,deltaB0]).T
    smv=np.array([smA,smB]).T
    rmv=np.array([rm,rm]).T
    zmv=np.array([zm,zm]).T
    one1=np.array([1,1]).T
    one2=np.array([0,1]).T
    A0=one1*rmv+one2*zmv+1/(tau*q)*Vmat.dot(smv-(1-q)*d0)
    ## our version that mimicks duffie
    if (1==0):
      qA=q*0.5
      qB=q*0.5
      qmat=diag([qA,qB])
      Vmat=diag(diag(Vmat))
      A0=one1*rmv+one2*zmv+1/tau*(inv(qmat)*Vmat).dot(smv-(1-q)*d0)
...
def funSolveUniqueDuffie(para,niter=100):
    ...
    #v=dfsave[:-2].as_matrix().T.flatten()
...
def funCS(paravary,para,T=15.,shock=.5,t_announce=1,t_implement=1,**simkargs):
    # T=10;simniter=1e3
  sims=zeros((size(paravary),int(T),4))
  for ind_para in range(0, size(paravary)):
    para.update(paravary[ind_para])
    solntup, solncore=funSolveUnique(para,niter=1)
    if type(solncore)==pandas.core.frame.DataFrame:
      sims[ind_para]=funSimulateEV(solntup,T=T,shock=shock,t_announce=t_announce,t_implement=t_implement,**simkargs)[['yA','yB','ErxA','ErxB']]
  ...
  psims=Panel(condsims,items=index,minor_axis=['yA','yB','ErxA','ErxB'])
  df.to_clipboard()
  return df, psims
...
def funSimulateEVDuffie(sol_tup, bshock=0, T=25, shock=.3,t_announce=10,t_implement=10, ycTenors=None,returnX=None,bse=0,seiter=1e4,decompose=0):
  # sol_tup=solntup; bshock=0; T=7; shock=1;t_announce=2;t_implement=2;ycTenors=array([1,2,5,10])
  para=sol_tup[2]
 ...
 # VarA=(thetaA/(1-thetaA)*alphaA1).T.dot(Sigma).dot(thetaA/(1-thetaA)*alphaA1);
  ...
  Erx=np.array([ErxA,ErxB])
  B=tau*inv(Vmat).dot(Erx)
  #bA=tau*ErxA*inv(Vmat)
  #bB=tau*ErxB*inv(Vmat)
  bA,bB=vsplit(B,2)
  bA=bA[0]
  bB=bB[0]
